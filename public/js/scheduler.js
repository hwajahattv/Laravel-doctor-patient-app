(function ($) {
    ...
    Scheduler.DEFAULTS = {
        locale: 'en', // i18n
        accuracy: 2, // how many cells of an hour
        data: [], // selected cells
        footer: true,
        multiple: true,
        disabled: false,
    ...
    Scheduler.TEMPLATES = {
        HALF_DAY_ROW: '<tr>' +
            ...
            '</tr>',
        HOUR_HEAD_CELL: '<th style="border:1 px solid white; border-radius: 5px" class="scheduler-hour-toggle" data-hour-toggle="%s" colspan="%s">%s</th>',
        DAY_ROW: '<tr data-index="%s"><td class="scheduler-day-toggle" data-day-toggle="%s">%s</td>%s</tr>',
        HOUR_CELL: '<td style="border:1 px solid white; border-radius: 5px" class="scheduler-hour%s" data-row="%s" data-col="%s"></td>',
    ...
    proto.end = function () {
        ...
        console.log(this.val());
        $("#testoutput").val(JSON.stringify(this.val()));
    };
